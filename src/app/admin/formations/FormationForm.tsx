...
export default function FormationForm({ defaultValues, formationId }: Props) {
  ...
  async function onSubmit(data: FormData) {
    ...
    const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...data, prix: Number(data.prix) }) })
    setLoading(false)
    if (res.ok) {
      setToast({ type: 'success', message: formationId ? 'Formation mise à jour !' : 'Formation créée avec succès !' })
