...
const atouts = [
  { icon: GraduationCap, titre: 'Apprenants certifiés', desc: "Nos candidats obtiennent des certifications officielles Goethe, TELC ou ÖSD — garantie d'un niveau d'allemand vérifié et reconnu." },
  { icon: Globe, titre: 'Profils motivés & sélectionnés', desc: "Chaque apprenant a choisi d'apprendre l'allemand avec un objectif professionnel clair. Leur motivation est un atout majeur pour l'intégration en entreprise." },
  { icon: Users, titre: 'Accompagnement culturel', desc: 'Au-delà de la langue, nos formations intègrent la culture, les codes professionnels et les usages du monde du travail germanophone.' },
  { icon: Briefcase, titre: "Prêts pour l'Ausbildung", desc: "Nos apprenants de niveau B1/B2 sont spécifiquement préparés aux exigences de l'Ausbildung et aux procédures de visa de travail." },
]
...
export default function RecruteursPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    setError('')
    try {
      const res = await fetch('/api/recruteurs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
      if (res.ok) { setSuccess(true) } else { setError('Une erreur est survenue. Veuillez réessayer.') }
        ...
        <div className="max-w-7xl mx-auto px-4">
          <Reveal animation="fade-up">
            <div className="text-center mb-14">
              ...
              <p className="section-subtitle">Nos apprenants ne sont pas de simples candidats — ce sont des professionnels en devenir, formés avec rigueur et passion pour réussir en Allemagne.</p>
            </div>
          </Reveal>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {atouts.map(({ icon: Icon, titre, desc }, i) => (
              <Reveal key={titre} animation="fade-up" delay={i * 80} className="border border-gray-100 rounded-2xl p-6 hover:border-[#E8001C] hover:shadow-lg transition-all group">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <Reveal animation="fade-up">
            <div className="text-center mb-12">
              ...
              <p className="section-subtitle">Nos apprenants se préparent à intégrer des entreprises dans ces secteurs clés de l&apos;économie allemande.</p>
            </div>
          </Reveal>
        ...
        <div className="max-w-3xl mx-auto px-4">
          <Reveal animation="fade-up">
            <div className="text-center mb-12">
              ...
              <p className="section-subtitle">Décrivez votre besoin et nous vous recontacterons avec des profils adaptés dans les plus brefs délais.</p>
            </div>
          </Reveal>
          ...
          {success ? (
            <Reveal animation="zoom">
              <div className="bg-white rounded-2xl shadow-lg p-12 text-center border border-gray-100">
                ...
                <p className="text-gray-500 mb-2">Merci pour votre intérêt envers nos apprenants.</p>
                ...
                <div className="flex flex-wrap justify-center gap-4">
                  <Link href="/formations" className="btn-primary inline-flex items-center gap-2">Voir nos formations <ArrowRight size={16} /></Link>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                  <div className="grid md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input {...register('nom')} placeholder="Max Müller" className="w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E8001C]" />
                      {errors.nom && <p className="text-red-500 text-xs mt-1">{errors.nom.message}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Entreprise *</label>
                      <input {...register('entreprise')} placeholder="Müller GmbH" className="w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E8001C]" />
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input {...register('role')} placeholder="Responsable RH, Directeur..." className="w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E8001C]" />
                      {errors.role && <p className="text-red-500 text-xs mt-1">{errors.role.message}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Email professionnel *</label>
                      <input {...register('email')} type="email" placeholder="max@mueller-gmbh.de" className="w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E8001C]" />
                      {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email.message}</p>}
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Votre besoin en recrutement *</label>
      ...
      <section className="py-16 bg-[#1A1A2E] text-white text-center px-4">
        <div className="max-w-2xl mx-auto">
          <Reveal animation="fade-up">
            ...
            <p className="text-gray-400 mb-6">Contactez-nous directement par email pour toute demande urgente ou question spécifique.</p>
            <a href="mailto:contact@visumplusakademie.com?subject=Demande de recrutement" className="btn-outline-white inline-flex items-center gap-2">
